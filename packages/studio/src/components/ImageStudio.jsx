...
function ProviderBadge({ model }) {
  ...
  // For now, derive from model.family or id via providers.js helpers
  const provId = model?.family || inferProviderForModel(model);
  const provider = getProviderById(provId);
  const isActive = provider.id === getActiveProviderId();
  if (!provider || provider.id === 'muapi') return null;
  return (
    <span
      ...
      title={isActive ? `Active provider: ${provider.name}` : `Model runs on ${provider.name}`}
    >
      {provider.name}
    </span>
  );
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-2 border-t border-white/[0.03] relative">
            ...
            <div className="flex items-center gap-2 relative flex-wrap pb-1 md:pb-0">
              ...
              <div className="relative">
                ...
                >
                  <div className="w-4 h-4 bg-[#7c3aed] rounded flex items-center justify-center">
                ...
                <ProviderBadge model={currentModels.find((m) => m.id === selectedModelId)} />
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="text-[11px] font-semibold text-white/70 group-hover:text-[#7c3aed] transition-colors">
                    {selectedAr}
                  </span>
                </button>
              ...
              {showEffectBtn && (
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="text-[11px] font-semibold text-white/70 group-hover:text-[#7c3aed] transition-colors max-w-[140px] truncate">
                ...
                {[1, 2, 3, 4].map((num) => (
                  <button
                    ...
                    className={`w-7 h-7 flex items-center justify-center rounded-md text-[10px] font-black transition-all ${
                      batchSize === num
                        ? "bg-[#7c3aed] text-black shadow-lg shadow-[#7c3aed]/20"
            ...
            <button
              ...
              disabled={generating}
              className="bg-[#7c3aed] text-black px-4 py-2 rounded-md font-medium text-sm hover:bg-[#e5ff33] hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 w-full sm:w-auto shadow-lg shadow-[#7c3aed]/10 disabled:opacity-50 disabled:cursor-not-allowed z-10"
            >
              {generating ? (
                <>
