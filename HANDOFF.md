...
**Higgsfield (2026-07-14):** Cloud API adapter's 401 bug fixed — the old code posted to `/v2/<endpoint>` with a `{params:{...}}` wrapper, neither of which exist on the real API (verified by reading the actual `@higgsfield/client` v0.2.1 v2 client source, since its own README disagrees with its own code). Real wire format: `POST /<endpoint>` with flat input body, `GET /requests/<id>/status` to poll, `Authorization: Key ID:SECRET` header (this part was already right). `packages/core/src/adapters/higgsfield.js` + new `packages/core/test/higgsfield.test.js`, core suite green (55 tests). Also fixed the Codex "Connect Higgsfield MCP" button — it was passing a `--transport http` flag Codex's CLI doesn't have; now uses `--url`. **Still needs Luke:** one live paid Image-tab generation to confirm the fix end-to-end (restart the Vidmyo window first — main-process files changed), and clicking Connect Higgsfield MCP for Claude Code/Codex to confirm `claude mcp list`/OAuth actually works.
